...
const ROLES = ['Relationship Manager', 'Sourcing Manager', 'Digital Marketing', 'Customer Success Associate'];
...
export default function ManageApps() {
  ...
  const [apps, setApps] = useState([
    { id: 1, name: "Rahul Sharma", role: "Relationship Manager", email: "rahul.s@gmail.com", phone: "+91 98765 00112", status: "New", experience: "4 Years", date: "2026-04-01", education: "MBA Marketing" },
    { id: 2, name: "Priya Deshmukh", role: "Digital Marketing", email: "priya.d@outlook.com", phone: "+91 98220 55443", status: "Interview", experience: "2 Years", date: "2026-03-28", education: "B.Tech IT" },
    { id: 3, name: "Amit Verma", role: "Sourcing Manager", email: "amit.v@yahoo.com", phone: "+91 91234 99887", status: "Shortlisted", experience: "6 Years", date: "2026-04-02", education: "BBA" },
    { id: 4, name: "Sneha Patil", role: "Relationship Manager", email: "sneha.p@gmail.com", phone: "+91 98111 22333", status: "Hired", experience: "5 Years", date: "2026-03-15", education: "MBA Finance" },
  ]);
  ...
  const handleAddSubmit = (e) => {
    e.preventDefault();
    const newcomer = { ...newApp, id: Date.now(), date: new Date().toISOString().split('T')[0], education: "Not specified" };
    setApps([newcomer, ...apps]);
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredApps.length === 0 ? (
                      ...
                      <tr 
                        key={app.id} 
                        className={`hover:bg-slate-50 transition-colors group ${selectedApp?.id === app.id ? 'bg-blue-50/40' : ''}`}
                      >
                        <td className="px-6 py-4 cursor-pointer" onClick={() => setSelectedApp(app)}>
                          <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${selectedApp?.id === app.id ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                              {app.name.split(' ').map(n => n[0]).join('').substring(0,2)}
                            </div>
                            <div>
                              <p className={`font-bold transition-colors ${selectedApp?.id === app.id ? 'text-blue-700' : 'text-slate-900 group-hover:text-blue-600'}`}>{app.name}</p>
                        ...
                        <td className="px-6 py-4 cursor-pointer" onClick={() => setSelectedApp(app)}>
                          ...
                          <p className="text-xs text-slate-400 font-medium mt-0.5 flex items-center gap-1">Applied: {app.date}</p>
                        </td>
              ...
              {selectedApp ? (
                <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden animate-in slide-in-from-right-8 duration-300">
                  ...
                  <div className="bg-slate-900 p-8 text-white relative">
                    ...
                    <p className="text-blue-400 font-bold text-sm mt-1">{selectedApp.role}</p>
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-2 mb-6 border-b border-slate-100 pb-6">
                      ...
                      <button onClick={() => deleteApp(selectedApp.id)} className="flex-1 flex flex-col items-center justify-center gap-2 py-3 bg-slate-50 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-xl border border-slate-200 transition-colors">
                    ...
                    <div className="space-y-4 mb-8">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-slate-800">{selectedApp.education}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 mb-6">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-slate-400 font-medium">1.2 MB</span>
                      </div>
                      <button className="w-full py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700 hover:text-blue-600 hover:border-blue-300 transition-colors flex items-center justify-center gap-2">
              ...
              <form className="p-6 space-y-6" onSubmit={handleAddSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input required type="text" placeholder="e.g. John Doe" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:bg-white transition-colors text-sm font-medium"
                      value={newApp.name} onChange={e => setNewApp({...newApp, name: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700">Phone Number *</label>
                    <input required type="tel" placeholder="+91..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:bg-white transition-colors text-sm font-medium"
                      value={newApp.phone} onChange={e => setNewApp({...newApp, phone: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700">Email Address *</label>
                    <input required type="email" placeholder="john@example.com" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:bg-white transition-colors text-sm font-medium"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input required type="text" placeholder="e.g. 3 Years" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:bg-white transition-colors text-sm font-medium"
