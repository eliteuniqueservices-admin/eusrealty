...
const InteractiveCalculator = dynamic(() => import('@/components/InteractiveCalculator'), {
  ssr: false,
  loading: () => (
    <div className="h-96 w-full bg-slate-900/40 animate-pulse rounded-[2.5rem] border border-white/5 flex items-center justify-center">
      <div className="text-slate-500 font-bold text-sm tracking-wider uppercase">Loading Earnings Estimator...</div>
    </div>
  )
});
...
/* ─────────────────────────────────────────────
   MAIN CLIENT CAREERS COMPONENT
 ───────────────────────────────────────────── */
export default function CareersPageClient({ initialJobPosts }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterDept, setFilterDept] = useState('All');
  const [showApplyModal, setShowApplyModal] = useState(false);
  const [selectedJob, setSelectedJob] = useState(null);
  const [mounted, setMounted] = useState(false);
  const [jobs] = useState(initialJobPosts || []);
        ...
        <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden bg-[#040407] rounded-b-[2.5rem] md:rounded-b-[4rem] mx-2 md:mx-4 mt-2 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] border-b border-white/5">
          ...
          <div className="relative z-10 w-full max-w-5xl px-6 flex flex-col items-center">
            <div className="text-center space-y-6 w-full flex flex-col items-center">
              {/* Top Badge */}
              <div className="inline-flex items-center gap-2.5 px-5 py-2.5 bg-white/5 backdrop-blur-xl rounded-full text-xs md:text-sm font-bold border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.3)] mb-4">
              ...
              {/* Description */}
              <p className="text-base sm:text-lg md:text-xl text-slate-300 font-light max-w-3xl mx-auto leading-relaxed mt-6">
